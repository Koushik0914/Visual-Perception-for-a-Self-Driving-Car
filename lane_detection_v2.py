...
def sliding_window(img, nwindows=6, margin=100, minpix = 1, draw_windows=True):
    ...
    for window in range(nwindows):
        ...
        win_y_low = int(img.shape[0] - (window + 1) * window_height)
        win_y_high = int(img.shape[0] - window * window_height)
...
def draw_lanes(img, left_fit, right_fit, roi):
    ...
    left = np.array(np.transpose(np.vstack([left_fit, ploty])), dtype=np.int32)
    right = np.array(np.transpose(np.vstack([right_fit, ploty])), dtype=np.int32)
    center = np.mean([left, right], axis=0, dtype=np.int32)
